...
version = 'PCA_'+str(n)+'_kNN_'+str(neighbors)
f = open('sk_svm_result_'+version+'.txt', 'w')

if load == 1:
    clf = joblib.load('sklearn_svm_model'+'_PCA_'+str(n)+'_kNN_'+str(neighbors)+'.pkl')
else:
    ...
    joblib.dump(clf, 'sklearn_svm_model'+'_PCA_'+str(n)+'_kNN_'+str(neighbors)+'.pkl')
    t2 = time.time()
